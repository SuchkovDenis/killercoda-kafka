...
docker exec kafka-1 /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --topic test-topic --partitions 3 --replication-factor 3 2>/dev/null
python3 -c "
from confluent_kafka import Producer
import json
p = Producer({'bootstrap.servers': 'localhost:19092'})
for i in range(10):
    p.produce('test-topic', key=f'user-{i%3}', value=json.dumps({'event': f'action_{i}', 'user': f'user-{i%3}'}))
p.flush()
" 2>/dev/null
